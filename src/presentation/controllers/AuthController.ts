  ...
  async adminLogin(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (isSuperAdmin && admin.role !== 'SUPER_ADMIN') {
        res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Insufficient permissions for super admin access' });
        return;
      }
  ...
  async refreshToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (tokenData.userId) {
        ...
        if (!admin) {
          res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Admin not found' });
          return;
        }
        ...
        res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Invalid token data' });
        return;
      }
